<template>
	...
	<!--详情抽屉-->
	<el-drawer v-model="showDetail" :with-header="false" direction="btt" size="80%">
		<!--详情-->
		...
		<view class="panel">
			<view class="panel-box">
				...
				<p v-for="(str, i) in showList.priceStrs" :key="i" style="text-align: center">
					<span>{{ str.label }}</span>
			...
			<view class="panel-box">
				...
				<p v-for="(str, i) in showList.platformStrs" :key="i" style="text-align: center">
					<span>{{ str.label }}</span>
					<span style="color: red">({{ str.value }})</span>
				</p>
			</view>
	...
	<view class="content" :class="showList.disk.win.value ? 'win' : null">
		<view class="box-card" :body-style="{ padding: '0px' }">
			<view class="button-box">
				<el-button type="primary" @click="initData()">全部重置</el-button>
				<el-button type="primary" @click="showDetail = true" v-if="showList.price">查看详情</el-button>
			</view>
			<view class="error-msg" v-if="showList.info.error.length">
				<span>异常提示：</span>
				<p v-for="(v, i) in showList.info.error" :key="i">{{ v.msg }}</p>
			</view>
			<view v-for="(item, index) in array" :key="index">
				...
				<view class="header">
					...
					<el-button type="danger" size="small" @click="initData(item.id)">重置</el-button>
				</view>
				<!-- 选项渲染 -->
				<view class="item" :class="['out', 'in'].includes(item.id) ? 'item-num' : 'item-disk'" v-for="(v, i) in item.list" :key="item.id + i">
					<el-switch class="switch" v-model="v.value" v-if="!v.hidden && v.type === 'switch'" size="small" :inactive-text="v.name" />

					<view class="select-box">
						<view class="select-title" v-if="!v.hidden && v.type === 'select'">{{ v.name }}</view>
						<el-select style="width: 56px" v-model="v.value" v-if="v.type === 'select'" size="small">
							<view v-for="n in v.len" :key="v.id + n">
								<el-option :label="n - 1 + v.unit" :value="n - 1" v-if="Array.isArray(v.lenExclude) ? !v.lenExclude.includes(n - 1) : true" />
							</view>
						</el-select>
...
<script setup>
  ...
  import { Button as elButton } from 'mint-ui';
